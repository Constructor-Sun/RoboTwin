...
def save_contact_sheet(frames, output_path, columns=4):
    if not frames:
        return
...
def preview_camera(task_name, task_config, camera_name, seed, frames, fps, output_dir):
    ...
    for frame_idx in range(frames):
        ...
        task.setup_demo(now_ep_num=frame_idx, seed=frame_seed, **args)
        obs = task.get_obs()
        frame = obs["observation"][camera_name]["rgb"]

        image_path = os.path.join(output_dir, f"{camera_name}_{frame_idx:03d}_seed{frame_seed}.png")
        Image.fromarray(frame).save(image_path)
        saved_frames.append(frame)

        task.close_env(clear_cache=(frame_idx == frames - 1))

    sheet_path = os.path.join(output_dir, f"{camera_name}_contact_sheet.png")
    video_path = os.path.join(output_dir, f"{camera_name}_preview.mp4")
    save_contact_sheet(saved_frames, sheet_path)
    save_video(saved_frames, video_path, fps)
...
def parse_args():
    ...
    parser.add_argument("--task_config", type=str, default="demo_camera")
    parser.add_argument("--camera_name", type=str, default="head_camera")
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--frames", type=int, default=8)
    parser.add_argument("--fps", type=int, default=2)
    parser.add_argument("--output_dir", type=str, default="camera_preview")
