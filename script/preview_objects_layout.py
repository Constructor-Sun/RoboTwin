...
def load_task_args(task_name, task_config):
    ...
    args.setdefault("objects_layout", {})
    args["objects_layout"]["enabled"] = True
...
def preview_objects_layout(task_name, task_config, seed, episodes, output_dir, camera_name, fps):
    ...
    for episode_idx in range(episodes):
        ...
        summary = task.objects_layout_summary
        image_path = os.path.join(output_dir, f"episode{episode_idx:03d}_{camera_name}_objects_layout.png")
        Image.fromarray(frame).save(image_path)

        actor_summaries = []
        for actor_info in summary.get("applied_actors", []):
            actor_summaries.append(
                f"{actor_info['name']}:dxy={np.round(actor_info['delta_xy'], 3).tolist()},yaw={round(actor_info['yaw_delta_deg'], 3)}"
            )

        print(
            f"episode {episode_idx}: "
            f"distractors={summary.get('o1_distractor_count')}, "
            f"registered={summary.get('registered_actor_count')}, "
            f"applied={summary.get('applied_actor_count')}"
        )
        if actor_summaries:
            print("  " + " | ".join(actor_summaries))

        task.close_env(clear_cache=(episode_idx == episodes - 1))

    save_contact_sheet(frames, os.path.join(output_dir, f"{camera_name}_objects_layout_contact_sheet.png"))
    save_video(frames, os.path.join(output_dir, f"{camera_name}_objects_layout_preview.mp4"), fps)
    print(f"Saved objects layout preview to {output_dir}")


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--task_name", type=str, default="click_alarmclock")
    parser.add_argument("--task_config", type=str, default="demo_objects_layout")
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--episodes", type=int, default=5)
    parser.add_argument("--output_dir", type=str, default="objects_layout_preview")
